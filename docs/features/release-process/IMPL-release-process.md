...
- `agentide/.github/workflows/release.yml` — push to `main` job. `googleapis/release-please-action@v4` reads `release-please-config.json`. On Release PR merge: job runs `pnpm -r publish --access public --no-git-checks` then `git checkout -- packages/agentide/package.json` (Q14).
