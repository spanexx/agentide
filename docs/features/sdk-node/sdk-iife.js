...
var __SdkModule = (() => {
  ...
  // docs/features/sdk-node/ws-shim.cjs
  var require_ws_shim = __commonJS({
    "docs/features/sdk-node/ws-shim.cjs"(exports, module) {
  ...
  var import_ws = __toESM(require_ws_shim(), 1);
  var WsClient = class {
    url;
    token;
    maxBackoffMs;
    ...
    constructor(config) {
      ...
      this.maxBackoffMs = config.maxBackoffMs ?? 3e4;
      this.baseBackoffMs = config.baseBackoffMs ?? 1e3;
    }
    /**
          ...
          ws.on("message", (data) => {
            ...
            try {
              ...
              if (typeof parsed.type === "string") {
                this.emit("message", JSON.parse(JSON.stringify(parsed)));
              }
            } catch {
            }
          });
...
${end.comment}` : end.comment;
  ...
  // packages/sdk-node/src/manifest.ts
  var CAPABILITY_NAME_REGEX = /^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*$/i;
  ...
  function parseManifest(source) {
    ...
    if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
      try {
        ...
        throw makeError("MANIFEST_INVALID_JSON", "$", `invalid JSON: ${err.message}`);
      }
    }
    try {
      return parse(source);
    } catch (err) {
      throw makeError("MANIFEST_INVALID_YAML", "$", `invalid YAML: ${err.message}`);
    }
  }
  ...
  // packages/sdk-node/src/register.ts
  ...
  async function resolveHandlers(source) {
    if (typeof source === "string") {
  ...
  function matchCapabilities(manifest, handlers) {
    const matched = [];
    for (const cap of manifest.capabilities) {
      const handler = handlers[cap.name];
      if (handler === void 0) {
        throw new Error(
          `register: capability '${cap.name}' declared in manifest but no handler found (handlers: ${Object.keys(handlers).join(", ")})`
        );
      }
  ...
  // packages/sdk-node/src/invoke.ts
  function makeCallContext(callId, capability, token, sessionId) {
  ...
  function makeHandlerContext(app, call, logger) {
    return { app, call, log: logger };
  }
  async function invokeHandler(handler, input, ctx) {
    return handler(input, ctx);
  }
  async function dispatchIncoming(client, handlers, ctx, msg, logger) {
  ...
  // packages/sdk-node/src/lifecycle.ts
  function attachLifecycle(deps) {
    ...
    client.on("message", (msg) => {
      if (handlers.onMessage) {
        handlers.onMessage(JSON.parse(JSON.stringify(msg)));
      }
    });
  }
  function reRegisterAll(client, registered) {
  ...
  function trackRegistration(state, cap) {
    state.registered.set(cap.name, cap);
  }
  function clearRegistrations(state) {
  ...
  function createSdk(config) {
    ...
    if (config.manifest === void 0 || config.manifest === null) {
      throw new Error("sdk-node: config.manifest is required (path or inline object)");
    }
    if (config.handlers === void 0 || config.handlers === null) {
      throw new Error("sdk-node: config.handlers is required (path or inline map)");
    }
    const logger = makeLogger(false);
    const phase = { value: "init" };
    const registered = /* @__PURE__ */ new Map();
    const lifecycleState = { phase, registered };
    const sdkInternal = { client: null };
    ...
    function attachLifecycleToClient(c) {
      attachLifecycle({
        client: c,
    ...
    const sdk = {
      ...
      async invoke(name, input) {
        ...
        return invokeHandler(handler, input, ctx);
      },
      async disconnect() {
        if (sdkInternal.client !== null) {
