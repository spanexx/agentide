...
> not opened by either dev bootstrap (`start-gateway.mjs`) or the CLI
...
1. **The SDK's door is not opened by either dev bootstrap.** `pnpm run
   gateway` (`scripts/start-gateway.mjs`) and `agentide start` (CLI) both
...
gateway (no `start-gateway.mjs` shim)
...
| Thing | Before | After |
|---|---|---|
| `start-gateway.mjs` | `{adapterMcp, adapterWs}` only | adds `backendRuntimePort: 7350` |
...
- `scripts/start-gateway.mjs`: add `backendRuntimePort: 7350` to the
  `createPlatform` call. Update the banner to include `sdk :7350`.
