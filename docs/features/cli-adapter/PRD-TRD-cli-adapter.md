...
WebSocket adapter (port 7300, push+pull). Both serve programs, not people. The operator
...
(v1 schema: `gateway_url` + `token` only; unknown keys IGNORED), then an interactive
...
2 = pre-flight/connection failure (missing config, unreachable gateway, close 1009/1011,
`subscribe.error`); 3 = TLS/upgrade failure; 4 = `auth.error` (close 1008); 5 =
interrupted (Ctrl-C).
...
chmod 600`) per run. `path:` token files follow the same 0600 rule for the warning; a
...
platform --watch --json                        # S7: pure JSON stream
# no env/config, piped stdout                   → exit 2 (no default URL)     # S1
...
`--token <jwt|path:/...>`, `--args '<json>'`, `--session <id>`, `--json`, `--watch`,
`--topic <pattern>`, `--help`, `--version`. Env: `PLATFORM_GATEWAY_URL`,
