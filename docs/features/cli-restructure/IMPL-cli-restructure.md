...
## Surgical-change delivery note (2026-08-09) — global data-dir default

**What:** the CLI no longer drops `./.agentide/data` into every repo it runs in. Default ambient data dir is now a shared global store `<home>/.local/share/agentide/<repo-key>/data` (repo-key = sha256(repo-root|cwd), first 12 hex) — per-repo isolation without polluting the repo. Opt-in `data_dir = "repo"` in the operator config file restores the legacy per-directory folder. `--data-dir` flag > `AGENTIDE_DATA_DIR` env > config `data_dir` > global default. Design locked with the user (surgical-change; no migration code — operator deletes old `.agentide` dirs by hand).
- **Why:** discovered operator issue — the CLI (one-shot + shell) created `.agentide/` in every repo touched; state now lives in one XDG-style store keyed by repo (D-119, resolved in this note).
- **Files:** NEW `packages/agentide/src/data-dir.ts` (CID:data-dir-003..006 — `defaultDataDir` resolver, `repoRoot`, `repoKey`); `config.ts` (`RawConfigFile.data_dir` CID:data-dir-001 + `readDataDirSetting` CID:data-dir-002, `"global"` default when absent/missing file; `saveConfig` is line-merge → key survives token refreshes, tested); `cli.ts` (`runCliInner` resolves via `defaultDataDir(...)` replacing the hardcoded `./.agentide/data` fallback; `homedir` import); `shell.ts` (local `resolveDataDir` → shared `defaultDataDir`; `home` threaded into the line-processor ctx for the cd context message).
- **Tests/validation:** NEW `data-dir.test.ts` (8 tests — priority chain, repo-key stability/isolation, config repo-mode, saveConfig preserves `data_dir`, init banner shows the global path + no `.agentide` in cwd); `shell.test.ts` reworked to temp homes (global store must never touch the real `~/.local/share`) + asserts no `.agentide` folder created. Full suite green at the time of writing (1185 pass / 28 skip / 0 fail); `simulate.sh` 21/21 PASS after the S8 assertion moved to the pinned-env semantics (cd key-switch is unit-pinned in shell.test.ts). PRD-TRD S1/S8 amended with the 2026-08-09 note above; operator config schema note added in `agentide-cli-consumer` PRD-TRD (Scenario 1 + ConfigFile). D-119 written + resolved in `docs/drift.md`.
