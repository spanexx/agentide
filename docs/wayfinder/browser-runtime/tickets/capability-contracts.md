...
| `browser.navigate` | `{ url, tabId?, newTab?: boolean, waitUntil?: 'load' \| 'domcontentloaded' \| 'networkidle', timeout? }` (`newTab: true` opens a fresh tab; default false) | `{ tabId, url }` | `BROWSER_NO_CONTEXT` (false), `BROWSER_TAB_NOT_FOUND` (false), `BROWSER_NAVIGATION_TIMEOUT` (true), `BROWSER_NAVIGATION_FAILED` (false) |
