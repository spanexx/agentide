...
| 4 | `plugin-manager` | Install/update/uninstall plugins from a Plugin Manifest; dependency validation; plugin lifecycle. **PRD APPROVED 2026-07-27** — [`docs/features/plugin-manager/PRD-plugin-manager.md`](features/plugin-manager/PRD-plugin-manager.md). 10 grilling decisions locked in CONTEXT.md. Local-source install path only in v1; marketplace registry path stubbed. Persistent install state in `./data/installed-plugins.json`. Seven `plugin.*` events. Structured `{ code, message, details }` errors. Ready for TRD. | `event-bus`, `capability-registry` (plugins register capabilities here) | Terminology → Plugin Manifest; Agentide → Section 5 |
