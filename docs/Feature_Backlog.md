...
| 24 | `websocket-adapter` | WebSocket adapter (universal push + pull). New package `@platform/adapter-websocket` — flat `{type, ...}` 16-frame envelope (auth/subscribe/unsubscribe/invoke + auth.ok/subscribe.ok/event/invoke.result/invoke.error/invoke.partial/invoke.end/stats/error); JWT-in-first-message-after-onopen auth with mid-connection refresh + `event.connection.rotated` audit; per-token `expectedOrigins` origin binding (RFC 6125 §6.4.3 single-label `*.` wildcard, deny-by-default for browsers, Node bypass); verbatim event-bus topic subscriptions with per-pattern authz (`platform.<first>.read`); universal `invoke` (call + stream) passing the kernel token through `gateway.handleInvocation`; per-connection 1 MiB FIFO backpressure with `{type:"stats", dropped:N}` recovery; 1 MiB inbound + outbound frame cap (close 1009); 30s ping / 10s pong heartbeat (close 1011); 30s pre-auth timeout (close 1008); default ON in `createPlatform` (port 7300), CLI opts out (`adapterWs: false`). **SHIPPED 2026-08-03** — see [`packages/adapter-websocket/`](../../packages/adapter-websocket) and [`docs/features/websocket-adapter/`](features/websocket-adapter/). 30 vitest tests across the new pack + 3 wiring tests + 31/31 post-impl assertions at `packages/agentide/scripts/simulate-websocket-adapter.mjs`. Build/lint/typecheck/check-banned-types all clean. Drift review `.reports/2026-08-03-drift-final-websocket-adapter.md` — initial Minor Drift (16 items) all addressed; D-51 (validatePattern export) closed. Test-authenticity review `.reports/2026-08-03-drift-websocket-adapter-tests.md` — verdict: ship, tests real; D-53 (sim channel tag) fixed, D-54 logged. Cross-pack follow-up: `expectedOrigins` mint side (D-50) CLOSED 2026-08-03 — `gateway.issueToken`/`auth.token.issue` accept `expectedOrigins`, CLI has `--origin`/`--origins` ([`docs/features/expected-origins/`](features/expected-origins/)). backend-runtime browser-path enforcement (D-54) CLOSED 2026-08-03 — post-verify origin binding via shared `@platform/origin` primitive (extracted from gateway-core), `sdk.auth.error ORIGIN_MISMATCH` + close 1008, deny-by-default for browsers, Node bypass; 706/706 tests. Unblocks `dashboard-core` (BI[13]) + `cli-adapter` (BI[23]). | `gateway-core` | Agentide → Section 8 |
| 28 | `agentide-cli-consumer` | Remote consumer commands in the `agentide` binary over the websocket adapter — 5 aliases (`capabilities`/`sessions`/`plugins`/`status`/`health`), `invoke <cap> [--args] [--session] [--mode]`, `watch <alias> [--topic]`; TOML config (`gateway_url`+`token`, `path:` indirection, flag > env > config > prompt, unknown keys ignored); TTY-aware output (tables / key:value / pretty vs compact JSON) + locked exit codes 0–5; watch = snapshot once + NDJSON events until Ctrl-C (exit 5), one backpressure stderr warning; token hygiene (0600 perms warning). **SHIPPED 2026-08-03** — see [`packages/agentide/src/consumer.ts`](../../packages/agentide/src/consumer.ts) and [`docs/features/agentide-cli-consumer/`](features/agentide-cli-consumer/). 22 tests new across consumer/exit-codes (21 consumer + 6 remote-dispatch cli + EPROTO regression); post-impl sim 13/13 at `packages/agentide/scripts/simulate-cli-consumer.mjs` (real gateway, real binary). Drift review `.reports/2026-08-03-drift-agentide-cli-consumer.md` — Minor Drift; `--mode stream` aligned to PRD S4 (flag no-op, invoke proceeds), D-65 logged. REPLACES row 23. | `gateway-core`, `websocket-adapter` | Agentide → Section 8 |
| 29 | `agentide-client-credentials` | Machine identities for unattended backends — operator `agentide client create/grant/list/revoke/rotate/redeem` subcommands (create prints the plaintext secret exactly once to `<dataDir>/clients/.secret-<id>.txt` mode 0600; 5 creates/hour rate limit; revoke flips `revoked=true` → next `POST /oauth/token` returns 401 `client_revoked`; rotate issues a new secret with 5-min grace; grant/redeem = one-time registration codes, 5-min TTL); gateway `ClientService` (salted-hash storage, `verifyClient` lastUsedAt, active-token enforcement on invoke); `POST /oauth/token` handler in gateway-core (client_credentials + registration_code grants, 10 req/min per client_id brute-force limit) + optional OIDC auth-code dev stub (`--enable-oidc`, `GET /oauth/authorize` + `/oauth/callback`, one-shot codes, JWT via 302) wired through adapter-mcp; sdk-node `TokenRefresher` (auto-mint at connect, refresh 60s-before-expiry + 30s jitter, single in-flight refresh, exponential backoff, `client_revoked` → clean ws close + `onRevoked`, legacy `{token}` config still works). **SHIPPED 2026-08-05** — see [`docs/features/agentide-client-credentials/`](features/agentide-client-credentials/). New tests across `client-service.test.ts` (11), `client-store-{types,impl}.test.ts`, `client-capabilities.test.ts`, `oauth-token-handler.test.ts`, `oidc-handler.test.ts` (4), `refresher.test.ts` (6); full repo 989/989 vitest; build/lint/typecheck/check-banned-types clean. Commits `60544e4` (CLI subcommand) → `9a9b739` (SDK auto-refresh) → `4334c84` (OIDC). Drift: D-70 logged open (CLI-path client actions + token_mint audit rows not yet wired — cap-path actions audit via handleInvocation), D-71 resolved (IMPL SDK-shape prose vs shipped nested config). Unblocks: `agentide-cli-consumer` consumers that need non-human auth; `rest-adapter` (same token mint path). | `gateway-core`, `sdk-node`, `adapter-mcp`, `agentide` | GRILL/PRD-TRD/IMPL `docs/features/agentide-client-credentials/`; OAuth 2.1 client_credentials + auth-code |
| 23 | `cli-adapter` | Rust static binary (`platform`) over the websocket adapter — reads AND writes in v1; one generic `invoke` + 5 aliases (`capabilities`/`sessions`/`plugins`/`status`/`health`); TOML config (`gateway_url`+`token`, `path:` indirection, flag > env > config > prompt); TTY-aware output + exit codes 0–5; `--watch` on aliases (NDJSON events, no reconnect); crate at `crates/cli-adapter/`, precommit via `scripts/precommit-rust.sh`. **DELETED 2026-08-03** — superseded by row 28 (`agentide-cli-consumer`): the design ported verbatim to the TS `agentide` binary (same aliases, config precedence, exit-code ladder, watch semantics); `crates/cli-adapter/` removed, rust integration test + `scripts/precommit-rust.sh` retired, D-64 closed. The Rust crate never shipped a feature-pipeline run (CHARTED only, blocked on `websocket-adapter`). | `gateway-core`, `websocket-adapter` | Agentide → Section 8 |
...
| # | Topic slug | Scope | Depends on | Source doc |
|---|---|---|---|---|
| 13 | `dashboard-core` | `dashboard.view.*` read caps (thin in-process wrappers over read-tier platform caps, type platform, owner `dashboard`, permission `platform.dashboard.read`, tier read, session-less) + web dashboard UI served by the dashboard package's own static server (`dashboardPort` 7200); all data over adapter-websocket (`invoke` + `subscribe`/`event`); browser-held read-only origin-bound token (mint per page load). **Charting DONE 2026-08-03** — map + tickets D1–D5 at [`docs/wayfinder/dashboard-core/`](wayfinder/dashboard-core/map.md); all decisions locked (D1 views, D2 cap shape, D4 token, D3 UI); drift D-50 mint side CLOSED 2026-08-03 ([`docs/features/expected-origins/`](features/expected-origins/)) — dashboard D4's `gateway.issueToken({…expectedOrigins})` mint call works as locked; no dashboard code change needed. Execution starts after adapter-websocket ships | `adapter-websocket` (the only door — Q2/Q3 locks), `platform-capabilities` | Agentide → Section 14 |
