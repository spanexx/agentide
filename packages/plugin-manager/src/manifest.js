...
export function parseManifest(content, yaml) {
    let raw;
    try {
        raw = yaml.parse(content);
    }
    catch (err) {
        const pos = err && typeof err === "object" && "linePos" in err
            ? err.linePos?.[0]
            : undefined;
        throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, err instanceof Error ? err.message : String(err), {
            line: pos?.line ?? null,
            column: pos?.col ?? null,
        });
    }
...
function coerceManifest(raw) {
    if (!isYamlObject(raw)) {
        throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, "manifest root must be a YAML mapping", { expected: "object", got: Array.isArray(raw) ? "array" : typeof raw });
    }
...
function coerceTypeKey(value, type) {
    if (value === undefined || value === null)
        return undefined;
    if (!isYamlObject(value)) {
        throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, `manifest type key "${type}" must be an object`, { expected: "object", got: Array.isArray(value) ? "array" : typeof value, type });
    }
    const id = value.id;
    if (typeof id !== "string") {
        throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, `manifest type key "${type}" must have a string "id"`, { expected: "string", got: typeof id, field: "id", type });
    }
    ...
    if (type === "runtime") {
        ...
        if (entry !== undefined && typeof entry !== "string") {
            throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, `manifest runtime.entry must be a string (path to a Node ESM module)`, { expected: "string", got: typeof entry, field: "entry" });
        }
...
function coerceVersion(value) {
    if (typeof value !== "string") {
        throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, "manifest must have a string \"version\"", { expected: "string", got: typeof value, field: "version" });
    }
    return value;
}
function coerceCapabilities(value) {
    if (value === undefined || value === null)
        return undefined;
    if (!Array.isArray(value)) {
        throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, "manifest \"capabilities\" must be an array of strings or {name,tier} objects", { expected: "array", got: typeof value, field: "capabilities" });
    }
    const out = [];
    for (let i = 0; i < value.length; i++) {
        ...
        if (!isCapabilityObject(item)) {
            throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, `manifest capabilities[${i}] must be a string or {name,tier} object`, { expected: "string|object", got: typeof item, index: i });
        }
        const validTiers = ["read", "act", "destructive"];
        if (item.tier !== undefined && !validTiers.includes(item.tier)) {
            throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, `manifest capabilities[${i}] tier must be one of read|act|destructive (got ${JSON.stringify(item.tier)})`, { index: i, got: String(item.tier) });
        }
        out.push({
            name: item.name,
            tier: (item.tier ?? "act"),
...
function coerceMetadata(value) {
    if (value === undefined || value === null)
        return undefined;
    if (!isYamlObject(value)) {
        throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, "manifest \"metadata\" must be an object of string → string", { expected: "object", got: typeof value, field: "metadata" });
    }
    const out = {};
    for (const [k, v] of Object.entries(value)) {
        if (typeof v !== "string") {
            throw new PluginManagerError(ERROR_CODES.MANIFEST_INVALID, `manifest metadata["${k}"] must be a string`, { expected: "string", got: typeof v, key: k });
        }
        out[k] = v;
    }
    return out;
...
export function validateManifest(manifest) {
    ...
    if (types.length === 0) {
        throw new PluginManagerError(ERROR_CODES.TYPE_MISSING, "manifest must declare exactly one of: runtime, service, developer", { expected: "one type key", got: "none" });
    }
    if (types.length > 1) {
        throw new PluginManagerError(ERROR_CODES.TYPE_AMBIGUOUS, "manifest must declare exactly one of: runtime, service, developer", { expected: "one type key", found: types });
    }
    const type = types[0];
    const id = manifest[type].id;
    if (!id) {
        throw new PluginManagerError(ERROR_CODES.ID_MISSING, `manifest ${type} plugin must have a non-empty "id"`, { type });
    }
    if (!manifest.version) {
        throw new PluginManagerError(ERROR_CODES.VERSION_MISSING, "manifest must have a non-empty \"version\"", {});
    }
    if (manifest.capabilities) {
        for (let i = 0; i < manifest.capabilities.length; i++) {
            ...
            if (!CAPABILITY_NAME_REGEX.test(capName)) {
                throw new PluginManagerError(ERROR_CODES.CAPABILITY_NAME_INVALID, `capability "${capName}" is not in the required format`, { capability: capName, format: "domain.action", index: i });
            }
        }
    }
}
...
export function manifestType(manifest) {
    if (manifest.runtime)
        return "runtime";
    if (manifest.service)
        return "service";
    if (manifest.developer)
        return "developer";
    throw new PluginManagerError(ERROR_CODES.TYPE_MISSING, "manifest has no type key", {});
}
// CID:manifest-004 - manifestId
...
export function manifestId(manifest, type) {
